...
| M10 | `blocked`  | Codex `/root` | 2026-07-24 | —          | [M10 evidence](evidence/M10.md), [hosted Windows gate](https://github.com/Williem3/FormationLap/actions/runs/30132034405), [Windows candidate checklist](evidence/M10_WINDOWS_RELEASE_CHECKLIST.md), [release threat checklist](../security/M10_RELEASE_THREAT_CHECKLIST.md), [dependency audit](../security/M10_DEPENDENCY_AUDIT.md) | Generate and back up the preview updater key, configure its environment inputs, and prepare a reviewed `v0.x` candidate; signed v1 remains deferred |
...
- Versions, tags, GitHub releases, and updater keys remain unchanged.
...
- The GitHub repository has an empty `release` environment. Signed v1 still
  ...
  OIDC.
- A technical preview requires generating and backing up the free Tauri updater
  key, configuring the protected `preview` environment inputs, and later
  obtaining explicit approval to create/push a selected `v0.x` tag.
