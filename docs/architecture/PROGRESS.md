...
- Repository visibility, versions, tags, GitHub releases, environments, and
  updater keys remain unchanged pending explicit publication approval.
...
- The GitHub repository has an empty `release` environment but remains private;
  its current plan rejected protection rules. Signed v1 still requires a
  protected environment, a production Tauri updater key, and either approved
  SignPath Foundation integration or Microsoft Artifact Signing/Azure OIDC.
- A technical preview requires explicit approval to make the repository public,
  create the protected `preview` environment, generate and back up the free
  Tauri updater key, and later create/push a selected `v0.x` tag.
...
- Making the repository public or creating any preview, Beta, or Stable tag or
  GitHub release remains an explicit external publication action.
