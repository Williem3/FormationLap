...
| M10 | `blocked`  | Codex `/root` | 2026-07-24 | —          | [M10 evidence](evidence/M10.md), [hosted Windows gate](https://github.com/Williem3/FormationLap/actions/runs/30132034405), [Windows candidate checklist](evidence/M10_WINDOWS_RELEASE_CHECKLIST.md), [release threat checklist](../security/M10_RELEASE_THREAT_CHECKLIST.md), [dependency audit](../security/M10_DEPENDENCY_AUDIT.md) | Configure protected signing inputs and qualify a signed Beta on Windows 10/11 |

## Current work

M10 is blocked after repository and unsigned local/hosted preflight, owned by
Codex `/root`.

- Repository release hardening, documentation, dependency audits, accessibility
  contracts, branded NSIS preflight, exact bundle surface, and final screenshot
  evidence are complete.
- The focused M10 commits are published to the authorized GitHub `master`
  branch. The clean-checkout
  [Windows version-one gate](https://github.com/Williem3/FormationLap/actions/runs/30132034405)
  passed at commit `897300f`.
- No repository behavior slice remains ready without production signing
  configuration and signed-candidate authorization.
- Hosted Authenticode/Tauri signing, signed Beta updater qualification, and the
  Windows 10/11 matrix remain true external seams and are not simulated.
...
- The GitHub repository currently has no `release` environment, repository
  signing variables, or signing secrets. M10 requires protected environment
  reviewers, a supplied or explicitly authorized production Tauri updater key,
  and Microsoft Artifact Signing/Azure OIDC configuration.
...
- Creating a Beta or Stable GitHub release/tag remains an explicit external
  publication action and has not been authorized.
