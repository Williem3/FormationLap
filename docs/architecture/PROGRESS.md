...
| M10 | `blocked`  | Codex `/root` | 2026-07-24 | —          | [M10 evidence](evidence/M10.md), [hosted Windows gate](https://github.com/Williem3/FormationLap/actions/runs/30132034405), [Windows candidate checklist](evidence/M10_WINDOWS_RELEASE_CHECKLIST.md), [release threat checklist](../security/M10_RELEASE_THREAT_CHECKLIST.md), [dependency audit](../security/M10_DEPENDENCY_AUDIT.md) | Confirm the updater-key passphrase is in the maintainer's password manager, then prepare a reviewed `v0.x` candidate; signed v1 remains deferred |

## Current work

M10 is blocked after completing the approved no-cost technical-preview slice,
owned by Codex `/root`.

- A separate, manual-only `v0.x` workflow now publishes an exact
  unsigned-Authenticode prerelease contract with explicit disclosure, Tauri
  updater signing, checksums, SBOM, licenses/notices, and provenance.
- The signed workflow excludes `v0.x`, requires `v1` or later, and retains every
  Authenticode and signed-candidate gate.
- Product, architecture, ADR, security, release, development, troubleshooting,
  evidence, and public install documentation now distinguish previews from
  signed Beta/Stable releases.
- `pnpm.cmd verify`, Rust formatting/all-feature Clippy, and 117 serial Rust
  tests passed; the one separately evidenced manual UAC test remains ignored.
- Commits `d3b4788` and `2273a53` are published on `master`; the repository is
  public, and the `preview` environment requires maintainer approval and accepts
  only `v0.*` deployment tags.
- The encrypted Tauri updater private key and public key are backed up outside
  the repository. The `preview` environment now contains both private-key
  secrets and the matching `FORMATION_LAP_UPDATE_PUBLIC_KEY` variable.
- Versions, tags, and GitHub releases remain unchanged.
...
- The GitHub repository has an empty `release` environment. Signed v1 still
  ...
  OIDC.
- A technical preview requires confirmation that the updater-key passphrase was
  saved in the maintainer's password manager, preparation and review of a
  synchronized `v0.x` candidate, and explicit approval to create/push its tag.
