...
| M10 | `blocked`  | Codex `/root` | 2026-07-24 | —          | [M10 evidence](evidence/M10.md), [preview infrastructure gate](https://github.com/Williem3/FormationLap/actions/runs/30136985750), [Windows candidate checklist](evidence/M10_WINDOWS_RELEASE_CHECKLIST.md), [release threat checklist](../security/M10_RELEASE_THREAT_CHECKLIST.md), [dependency audit](../security/M10_DEPENDENCY_AUDIT.md) | Review local candidate commit `0bb2c75`, then explicitly authorize pushing it; no tag, workflow dispatch, or release is authorized |

## Current work

M10 is blocked after preparing the local technical-preview candidate, owned by
Codex `/root`.

- Local commit `0bb2c75` synchronizes `0.9.0-preview.1` across the package,
  Cargo manifest/lockfile, and Tauri configuration.
- The candidate release notes and README identify the unsigned Authenticode
  tier and explain SmartScreen, unknown-publisher, non-Stable, checksum, and
  provenance expectations.
- A repository-level contract requires the matching
  `v0.9.0-preview.1` candidate tag shape and disclosure-complete notes.
- `pnpm.cmd verify`, npm audit, Rust formatting/all-feature Clippy, 118 serial
  Rust tests, RustSec audit, and the native debug build passed at the candidate
  version.
- The updater key remains only in the protected GitHub environment and its
  encrypted external backup. No secret material entered the workspace.
- The candidate commit is local only. No push, tag, workflow dispatch, or
