...
| Unsigned or foreign-signed main/helper execution    | Workflow and runtime WinVerifyTrust must succeed; locked main/helper hashes and one equal signer-certificate SHA-256 must match the release-key-signed identity manifest                              | Release identity module, workflow, and adversarial tests       | Controlled |
| Installer contents differ from signed binaries      | Bundler consumes the signed executable and signed sidecar; installer is signed afterward and must have the same approved signer-certificate SHA-256 as main/helper                                    | Release workflow contract                                      | Controlled |
...
| Unsafe executable configuration runs unreviewed     | Imports, complete privileged profiles, edited profiles, and forged JSON fail closed to Needs Review when any executable requires repair; Session start independently recomputes that state            | ProfileLibrary, Core, and React review tests                   | Controlled |
...
| Corrupt live state destroys its only valid backup   | Settings and Session journals validate compatible backups first, preserve corrupt bytes in two bounded slots, and restore a synchronized copy without consuming the backup; newer schemas fail closed | Core recovery integration tests and `atomic_file.rs`           | Controlled |
...
| Automatic checks occur without consent              | New settings default off, explicit stored true is preserved, and manual Check now consents to one check                                                                                               | UpdateAdvisor and Settings behavior tests                      | Controlled |
| Update task overlaps race-safe Session start        | Native UpdateCoordinator cancels and joins checks before the core transition; install and Session activity hold mutually exclusive native/core leases                                                 | UpdateCoordinator and FormationLapCore tests                   | Controlled |
| Updater follows attacker-controlled metadata        | Native updater accepts only exact official repository/tag/version/x64 filenames, controlled HTTPS redirects, and bounded metadata/installer bodies                                                    | Native updater and bundle-surface tests                        | Controlled |
| Verified installer is replaced before UAC launch    | Staging holds a read-share-only Windows file handle from verified write through ShellExecute process creation                                                                                         | Native updater staging/launch implementation                   | Controlled |
...
- Required pull-request review/check rules and secret-push protection.

Repository tests prove only the checked-in workflow controls. They cannot prove
that GitHub has configured the protected `release` environment, its reviewers
and production secrets, branch/ruleset protections, or secret-push protection.
Those controls remain required and pending until independently evidenced.
