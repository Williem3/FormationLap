...
function verifyChecksums(directory, expectedFiles) {
  ...
  for (const line of readFileSync(join(directory, "SHA256SUMS.txt"), "utf8")
    .split(/\r?\n/)
    .filter(Boolean)) {
...
function verifyUpdaterMetadata(
  ...
  if (metadata.version !== version) {
    throw new Error("Updater metadata version does not match the preview.");
  }
  if (typeof metadata.notes !== "string" || !/unsigned/i.test(metadata.notes)) {
    throw new Error("Updater metadata must disclose the unsigned preview.");
  }
  if (
...
function verifySbom(sbom, version) {
  if (
    ...
    throw new Error("SPDX SBOM does not identify this Formation Lap preview.");
  }
}
...
function verifyUnsignedDisclosure(path) {
  const disclosure = readFileSync(path, "utf8");
  for (const phrase of [
    "unsigned technical preview",
    "do not have Windows Authenticode publisher signatures",
    "SmartScreen",
    "Unknown publisher",
    "not a Stable Formation Lap release",
    "SHA256SUMS.txt",
    "Tauri updater signature",
  ]) {
    if (!disclosure.includes(phrase)) {
      throw new Error(
        `UNSIGNED-PREVIEW.txt is missing required disclosure: ${phrase}`,
      );
    }
  }
}
...
function verify() {
  ...
  if (!existsSync(directory) || !statSync(directory).isDirectory()) {
    throw new Error("Preview artifact directory does not exist.");
  }
  ...
  verifyUnsignedDisclosure(join(directory, "UNSIGNED-PREVIEW.txt"));
  verifyChecksums(directory, artifactNames);
  console.log(
    `Unsigned preview artifact contract passed: ${basename(directory)} ${version}`,
  );
}
...
try {
  ...
  console.error(
    `Preview artifact contract failed: ${
