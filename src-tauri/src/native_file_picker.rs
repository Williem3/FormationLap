...
const EXECUTABLE_FILTER: &str = "Executable files\0*.exe\0\0";
...
pub(crate) fn pick_executable_path() -> std::io::Result<Option<String>> {
    ...
    let filter = EXECUTABLE_FILTER.encode_utf16().collect::<Vec<_>>();
...
#[cfg(test)]
mod tests {
    use super::EXECUTABLE_FILTER;

    #[test]
    fn picker_limits_selection_to_executables() {
        assert_eq!(EXECUTABLE_FILTER, "Executable files\0*.exe\0\0");
    }
}
